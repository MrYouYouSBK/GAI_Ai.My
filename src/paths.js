...
//
// Electron 主进程启动时会通过 GAI 环境变量注入这两个路径。
// 旧 BaiLongma 名称只在 compat 层读取，用于无损升级。
//
...
import { getCompatibleResourcesDirEnv, getCompatibleUserDirEnv } from './compat/legacy-bailongma.js'

const __dirname = path.dirname(fileURLToPath(import.meta.url))
const REPO_ROOT = path.resolve(__dirname, '..')

const compatibleUserDir = getCompatibleUserDirEnv()
const compatibleResourcesDir = getCompatibleResourcesDirEnv()

const USER_DIR = compatibleUserDir ? path.resolve(compatibleUserDir) : REPO_ROOT
const RESOURCES_DIR = compatibleResourcesDir ? path.resolve(compatibleResourcesDir) : REPO_ROOT
